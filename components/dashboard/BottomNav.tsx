...
export function BottomNav() {
  ...
  return (
    <div className="lg:hidden" style={{ position: 'fixed', bottom: 0, left: 0, right: 0, zIndex: 40, background: 'rgba(13,31,54,0.98)', backdropFilter: 'blur(12px)', borderTop: '1px solid rgba(255,255,255,0.08)', paddingBottom: 'env(safe-area-inset-bottom)' }}>
      <div style={{ display: 'flex', alignItems: 'flex-end', justifyContent: 'space-around', padding: '6px 8px 8px' }}>
        {NAV_ITEMS.map((item) => {
          const active = pathname === item.href
          if (item.isFab) {
            return (
              <button key={item.href} onClick={() => {
                if (pathname === '/dashboard') { window.dispatchEvent(new CustomEvent('zafi:voice-overlay')) } else { router.push('/dashboard') }
              }} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 2, background: 'none', border: 'none', cursor: 'pointer', marginTop: -16, padding: 0 }}>
                <div style={{ width: 48, height: 48, borderRadius: '50%', background: '#2563EB', display: 'flex', alignItems: 'center', justifyContent: 'center', boxShadow: '0 2px 12px rgba(37,99,235,.4)' }}>
                  <NavIcon icon={item.icon} active={false} />
                </div>
                <span style={{ fontSize: 9, color: 'rgba(255,255,255,0.4)', fontWeight: 500 }}>{item.label}</span>
              </button>
            )
          }
          return (
            <Link key={item.href} href={item.href} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 2, textDecoration: 'none', minWidth: 48 }}>
              ...
              <span style={{ fontSize: 9, fontWeight: 600, color: active ? '#60A5FA' : 'rgba(255,255,255,0.4)', letterSpacing: '0.02em' }}>{item.label}</span>
              {active && <div style={{ width: 20, height: 2, borderRadius: 9999, background: '#60A5FA', marginTop: -1 }} />}
            </Link>
          )
        })}
      </div>
