...
function generateInsights(profile: FinancialProfile, components: ScoreBreakdown['components']): string[] {
  ...
  if (components.debtBurden < 14)
    insights.push(`Tus deudas representan ${Math.round((debt/inc)*100)}% de tu ingreso. Prioriza pagarlas.`);
  ...
  if (components.expenseRatio < 10)
    insights.push(`Tus gastos fijos consumen más del 60% de tu ingreso (${Math.round((exp/inc)*100)}%). Busca reducirlos.`);

  if (insights.length === 0)
    insights.push('¡Vas bien! Mantén el hábito de revisión mensual para seguir mejorando.');
